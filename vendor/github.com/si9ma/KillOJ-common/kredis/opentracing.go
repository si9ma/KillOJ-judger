...
func WrapRedisClient(ctx context.Context, client *redis.ClusterClient) *redis.ClusterClient {
	if ctx == nil {
		return client
	}
...
func process(parentSpan opentracing.Span, opts *redis.ClusterOptions) func(oldProcess func(cmd redis.Cmder) error) func(cmd redis.Cmder) error {
...
func processPipeline(parentSpan opentracing.Span, opts *redis.ClusterOptions) func(oldProcess func(cmds []redis.Cmder) error) func(cmds []redis.Cmder) error {
...
func doSpan(parentSpan opentracing.Span, opts *redis.ClusterOptions, operationName, dbMethod string) {
	tracer := parentSpan.Tracer()
	span := tracer.StartSpan(operationName, opentracing.ChildOf(parentSpan.Context()))
	defer span.Finish()
	ext.DBType.Set(span, "redis")
	ext.PeerAddress.Set(span, strings.Join(opts.Addrs, ","))
